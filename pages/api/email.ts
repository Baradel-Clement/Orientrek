...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    const mailToOrientrek = await resend.emails.send({
      ...
      subject: `La demande de ${req.body.firstName} ${
        req.body.lastName
    ...
    const mailToClient = await resend.emails.send({
      ...
      Orientrek a pris en compte votre demande et vous remercie. Notre équipe met tout en œuvre pour y répondre avant le ${datePlus3.getDate()} ${
